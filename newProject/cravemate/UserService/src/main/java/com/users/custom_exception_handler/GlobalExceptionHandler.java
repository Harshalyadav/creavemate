package com.users.custom_exception_handler;
...
public class GlobalExceptionHandler {
	...
	// method level anno to tell SC , following is an exc handling method : to
		...
		public ApiResponse handleUserAlreadyExistsException(
				ResourceNotFoundException e) {
			System.out.println("in user exists " + e);
			return new ApiResponse(e.getMessage());
		}
	
		
		// method level anno to tell SC , following is an exception handling method : to
				// handle : authentication failure
				@ExceptionHandler(AuthenticationException.class)
				@ResponseStatus(value = HttpStatus.UNAUTHORIZED)
				public ApiResponse handleAuthenticationException(
						AuthenticationException e) {
					System.out.println("in auth failure " + e);
					return new ApiResponse(e.getMessage());
				}
